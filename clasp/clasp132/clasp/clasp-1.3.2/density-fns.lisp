...
;;;; *-* File: Titanic: /usr/users/eksl/mac-files/clasp/clasp/development/density-fns.lisp *-*
;;;; *-* Last-edit: Friday, September 24, 1993  11:32:15; Edited-By: oates *-* 
...
;;;          head of file.  (Westy)
;;;
;;; **** error-function-complement-helper **** will not compile in MCL
;;;
;;; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
;;; --*--
  ...
  (let ((gln (gamma-ln a)))
	...
	(let* ((itmax 100)
	  ...
	  (dotimes (i itmax)
		...
		(let ((result (* sum (safe-exp (- (* a (log x)) x gln)))))
	...
	;; Use the continued fraction representation.  The following is the code
	;; of what Numerical Recipes in C calls ``GCF.'' Their code computes the
	;; complement of the desired result, so we subtract from 1.0 at the end.
	(let ((itmax 100)
	      (eps   3.0e-7)
	  ...
	  (dotimes (i itmax)
	    ...
	    (unless (zerop a1)
	      (setf fac (/ 1.0 a1)
		    g   (* b1 fac))
	      (if (< (abs (/ (- g gold) g)) EPS)
		  (let ((result (* (safe-exp (- (* a (log x)) x gln)) g)))
		    (return-from gamma-incomplete (values (- 1.0 result) gln)))
		  (setf gold g))))
...
compute and has fractional error everywhere less than 1.2x10^{-7}."
  ...
  (let* ((z   (abs x))
	 ...
	 (ans #+MCL (error "this function didn't compile")
	      #-MCL
	      (* y (exp (+ (* (- z) z)
			   -1.26551223
			   (* y
