...
(eval-when (compile load eval)
  #+(or (and :excl :allegro-v4.0) :mcl :lucid)
...
(defun parse-with-string-delimiter (delim string &key (start 0) end)
  ...
  #-lucid(declare (simple-string string))
...
(unless (fboundp (intern "NTH-VALUE" #+CLTL2 'common-lisp #-CLTL2 'lisp))
...
;;; ********************************
;; 10-16-92 Changed this to use an equalp hash table so that comparision is not case sensitive.  (Westy)
(defvar *physical-host-table* (make-hash-table :test #'equalp)
  ...
  (when local-hostab
	...
	(cond ((null line)
		 ...
		 (unless (or (equal "" system) (null system))
		   ...
		   (setf (physical-host-type (string-upcase name)) system)
		   (setf (physical-host-type (string-downcase name)) system)
...
(defvar *check-for-default-host-mismatch* nil)
...
(defun parse-generic-namestring (thing &optional host
  ...
  (let ((host-string (get-host-string thing ":"))
    ...
    ;; 10-16-92 Added *check-for-default-host-mismatch* because I have no idea
    ;; what the purpose of this code is.  (Westy)
    (when (and *check-for-default-host-mismatch* host host-string (not (string-equal host host-string)))
      (cerror "Ignore it."
...
;; 10-16-92 Added this for Allegro and 05-19-93 lucid.  (Westy)
#+(or allegro lucid)
(unless (fboundp 'old-merge-pathnames)
  (setf (symbol-function 'old-merge-pathnames)
	(symbol-function 'lisp::merge-pathnames))
  (defun lisp::merge-pathnames (pathname &optional (defaults *default-pathname-defaults*)
						   (default-version :NEWEST))
    (old-merge-pathnames (real-filename pathname) (real-filename defaults) default-version)))

#+lucid ;; 05-19-93 Added this. (Westy) 
(convert-file-function lisp::pathname)
