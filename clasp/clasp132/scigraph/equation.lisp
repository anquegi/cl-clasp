...
;; 9-21-93 Added this because example given in doc does not work without it. (Westy)

(defmethod initialize-instance :after ((self graph-sample-data-mixin) &key)
  (when (sample-data self)
    (compute self)))
...
(defmethod compute ((self histogram-data))
  ...
  ;; 9-21-93 Removed references to graph which is no longer a slot. (Westy)
  (with-slots (#+OLD graph min max bin-count bin-size bins sample-data) self
    #+OLD
    (and graph (setf (auto-scale-needed graph) t))
    (unless (and min max)
