  ...
  ((:clim-0.9
     (clim:stream-current-text-style 
		 (if (typep stream 'ci::encapsulating-stream-mixin)
		     ...
		     stream)))
   (:CLIM-1.0
    #+ignore (slot-value stream 'clim::current-text-style) ;fix for merged-text-styles bug AC 9/24/93
  ...
  (((not :clim)
   ...
   (:clim-1.0
    ...
    (clim:make-application-frame
     type
     ;; 9-15-93 Changed this to only hack it in Lucid. When `launch-frame' calls us with a parent we need to use it.
     :parent #+CRAP (find-frame-manager :if-exists :create) #-CRAP (or parent (find-frame-manager :if-exists :create))
     :pretty-name title
