...
(defun suggest-bin-directory (&optional (base *load-pathname*)
  ...
  (let ((platform ; 9-15-93 Added this for Allegro which uses .fasl on all platforms. (Westy)
	 #+MIPS             "MIPS"
	 #+SPARC            "SPARC"
	 #-(or MIPS SPARC)  nil
	 )
	(instruction-set
        ...
        (GUI
         ...
         #+clim-0.9                "CLIM-0-9"
         #+(and clim-1.0 (not clim-1.1)) "CLIM-1-0" ; in case both featues exist (Westy)
         #+clim-1.1                "CLIM-1-1"
         #+clim-2                  "CLIM-2"))
    (namestring (make-pathname
                 ...
                 (append
                  ...
                  (list (string-downcase
                         (format nil "~A-~@[~A-~]~A-~A"
                                 prefix 
				 platform ; 9-15-93 Added this for Allegro which uses .fasl on all platforms. (Westy)
                                 instruction-set
                                 gui))))))))

(portable-defsystem::defsystem dwim
