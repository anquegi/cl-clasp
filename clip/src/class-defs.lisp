...
(defclass experiment (named-object-mixin
                      remember-instances
                      object-with-properties)
  ((status :initform :idle)
   (trial-number
     :initform nil
     :documentation "In a run of trials, this is the number of the trial that is
 currently being run.")
   ...
   (end-of-trial-time
     ...
     :documentation "The absolute time at which a trial ends. At that time, the 
system is stopped and the `after-experiment-run' function is executed.")
   (output-file-name
     :initarg :output-file-name
     :initform nil
     :accessor output-file-name
     :documentation "Bound by `run-experiment' to the file used for data
 output.")
   (error-file-name
     :initform nil
     :initarg :error-file-name
     :accessor error-file-name
     :documentation "Bound by `run-experiment' to the file used for 
 error output.")
   ...
   (scenario
     ...
     :documentation "Bound to the scenario being utilitized by the current
 experiment.")
   (script-name
     :initarg :script-name
     :initform nil
     :accessor experiment.script-name
     :documentation "Bound to the script-name which is being used to guide the
 current experiment.")
   ...
   (deactivate-scheduled-function-hook
    :initarg :deactivate-scheduled-function-hook)
...
(defclass instrumentation (record-call-tree-mixin
                           named-object-mixin
                           remember-instances)
  ((status        :initarg :status       :reader status)
   (arguments     :initarg :arguments    :reader instr.arguments)
   (enable-function   :initform nil :initarg :enable-function
                      :reader instr.enable-function)
   (disable-function  :initform nil :initarg :disable-function
                      :reader instr.disable-function)
   (reset-function    :initform nil :initarg :reset-function
                      :reader instr.reset-function)
   (display-function  :initform nil :initarg :display-function
                      :reader instr.display-function)
   (report-function   :initform nil :initarg :report-function
                      :reader instr.report-function)
   ...
   (output-file-name :initarg :output-file-name :initform nil
...
(defclass composite-instrumentation-mixin
    (column-producing-instrumentation-mixin)
  ()
  (:metaclass named-class)
...
(defclass mapping-instrumentation-mixin (column-producing-instrumentation-mixin)
  ((map-function       :initarg :map-function :reader instr.map-function)
   ...
   (congruity-already-checked? :initform nil
                               :accessor instr.congruity-already-checked?))
  (:metaclass named-class))
...
(defclass composite-time-series-instrumentation
...
(defclass functional-composite-instrumentation
    (time-series-instrumentation-mixin
...
(defclass child-of-composite-instrumentation
